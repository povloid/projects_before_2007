...
S_POST_DBTable::S_POST_DBTable(QWidget *parent)
    : PAbstractDBTable(parent)
{
  captionViewLabel->setText( tr("Поставщики"));

  selectSQL = "SELECT KOD,NAME FROM S_POST ORDER BY KOD";
...
bool S_POST_DBTable::virtualAddRec( void )
{
  iobj_KOD->setValue(0);
  iobj_KOD->setFocus();
  ...
  return query.exec("INSERT INTO S_POST (KOD, NAME) VALUES ( '"+
		    QString::number(iobj_KOD->value())+"','"+
		    iobj_NAME->text()+
		    "')");
}

bool S_POST_DBTable::virtualEditRec( int & row )
{
  ...
  QSqlQuery query( db );
  return query.exec( "UPDATE S_POST SET KOD = '" + strKOD 
		     + "', NAME = '" + QString::number(iobj_KOD->value()) 
		     + "' WHERE KOD = '" + lastStrKOD + "'" );
}

bool S_POST_DBTable::virtualDelRec( int & row )
{
  QSqlRecord record = model->record( row );
  ...
  case 0:{
    QSqlQuery query( db );
    return query.exec("DELETE FROM S_POST WHERE KOD = '" 
		      +  record.value("KOD").toString() + "'");
  }
  case 1: 
    return true;
