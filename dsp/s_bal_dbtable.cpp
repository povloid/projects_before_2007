...
S_BAL_DBTable::S_BAL_DBTable(QWidget *parent)
    : PAbstractDBTable(parent)
{
  captionViewLabel->setText( tr("Баланс восфора"));


  selectSQL = "SELECT * FROM S_BAL ORDER BY KOD";
...
bool S_BAL_DBTable::virtualAddRec( void )
{
  iobj_KOD->setValue(0);
  iobj_KOD->setFocus();
  ...
  return query.exec("INSERT INTO S_BAL (KOD, NAME) VALUES ( '"+
		    QString::number(iobj_KOD->value())+"','"+
		    iobj_NAME->text()+
		    "')");
}

bool S_BAL_DBTable::virtualEditRec( int & row )
{
  ...
  QSqlQuery query( db );
  return query.exec( "UPDATE S_BAL SET KOD = '" + QString::number(iobj_KOD->value())
		     + "', NAME = '" + iobj_NAME->text() 
		     + "' WHERE KOD = '" + lastStrKOD + "'" );
}

bool S_BAL_DBTable::virtualDelRec( int & row )
{
  QSqlRecord record = model->record( row );
  ...
  case 0:{
    QSqlQuery query( db );
    return query.exec("DELETE FROM S_BAL WHERE KOD = '" 
		      +  record.value("KOD").toString() + "'");
  }
  case 1: 
    return true;
