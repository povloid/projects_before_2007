...
S_PPROST_DBTable::S_PPROST_DBTable(QWidget *parent)
    : PAbstractDBTable(parent)
{
  captionViewLabel->setText( tr("Причины простоев"));

  selectSQL = "SELECT * FROM S_PPROST ORDER BY KOD";
...
bool S_PPROST_DBTable::virtualAddRec( void )
{
  iobj_KOD->setValue(0);
  iobj_KOD->setFocus();
  ...
  return query.exec("INSERT INTO S_PPROST (KOD, NAME) VALUES ( '"+
		    QString::number(iobj_KOD->value())+"','"+
		    iobj_NAME->text()+
		    "')");
}

bool S_PPROST_DBTable::virtualEditRec( int & row )
{
  ...
  QSqlQuery query( db );
  return query.exec( "UPDATE S_PPROST SET KOD = '" + QString::number(iobj_KOD->value()) 
		     + "', NAME = '" + iobj_NAME->text() 
		     + "' WHERE KOD = '" + lastStrKOD + "'" );
}

bool S_PPROST_DBTable::virtualDelRec( int & row )
{
  QSqlRecord record = model->record( row );
  ...
  case 0:{
    QSqlQuery query( db );
    return query.exec("DELETE FROM S_PPROST WHERE KOD = '" 
		      +  record.value("KOD").toString() + "'");
  }
  case 1: 
    return true;
