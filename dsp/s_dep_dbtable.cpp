...
S_DEP_DBTable::S_DEP_DBTable(QWidget *parent)
    : PAbstractDBTable(parent)
{
  captionViewLabel->setText( tr("Цеха"));


  selectSQL = "SELECT * FROM S_DEP ORDER BY KOD";

  model->setQuery( selectSQL , db );
  model->setHeaderData(0, Qt::Horizontal, tr("Ном. цеха"));
  model->setHeaderData(1, Qt::Horizontal, tr("Наименование цеха"));
...
bool S_DEP_DBTable::virtualAddRec( void )
{
  iobj_KOD->setValue(0);
  iobj_KOD->setFocus();
  ...
  return query.exec("INSERT INTO S_DEP (KOD, NAME) VALUES ( '"+
		    QString::number(iobj_KOD->value())+"','"+
		    iobj_NAME->text()+
		    "')");
}

bool S_DEP_DBTable::virtualEditRec( int & row )
{
  ...
  QSqlQuery query( db );
  return query.exec( "UPDATE S_DEP SET KOD = '" + QString::number(iobj_KOD->value()) 
		     + "', NAME = '" + iobj_NAME->text()
		     + "' WHERE KOD = '" + lastStrKOD + "'" );
}

bool S_DEP_DBTable::virtualDelRec( int & row )
{
  QSqlRecord record = model->record( row );
  ...
  case 0:{
    QSqlQuery query( db );
    return query.exec("DELETE FROM S_DEP WHERE KOD = '" 
		      +  record.value("KOD").toString() + "'");
  }
  case 1: 
    return true;
