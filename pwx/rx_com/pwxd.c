...
// Здесь определено объединение ptype
// Суть такова: Данные полученные с порта COM1 записываются в него побайтно
// а потом реальные показания считываются по числами по 2 байта
...
// Переменные необходимые для работы с портом COM1
...
main()
{
    ...
    syslog(LOG_NOTICE,"Запуск программы pwx 1.0\nИнициализация устройства COM1, фаил устройства /dev/ttyS0 -->");                    
    //////////////////////////////////////////////////////////////////////////
	...
	syslog(LOG_NOTICE,"Инициализация Базы данных: %s ",OPT_SERVERNAME);
	...
	// printf("sizeof(query)=%d\n",sizeof(query));
     ...
     //Подключаемся к базе данных в два этапа:                                                        
				...
				//Далее идет проверка стандартная для большенства API-функций проверка и анализ результата   
					     ...
					     syslog(LOG_NOTICE,"1. Ошибка!\n");                                               
					     syslog(LOG_NOTICE,"%s",status_vector);                                     
					     syslog(LOG_NOTICE,"Приложение завершено с ошибкой.\n");                    
...
//-------------------------------------------------------------------------------------------
// Этот участок кода отвечает за запуск приложения как демона системы
// тоесть программа будет висеть в памяти сама по себе и выполняться
//	
	...
	if(fork())  		/*форкаемся*/                                                                     
                exit(0);        // В UNIX форк это порождение процесса                                                                      
...
//-------------------------------------------------------------------------------------------
	if (db_handle) isc_detach_database(status_vector,&db_handle);                         
				//Проверка на ошибку 
				     if (status_vector[0] == 1 && status_vector[1])                               
				         {                                                                            
				             printf("Ошибка отключения от базы данных! \n");                                               
				             isc_print_status(status_vector);                                     
				             printf("Приложение завершено с ошибкой.\n");                       
					     
					     syslog(LOG_NOTICE,"Ошибка отключения от базы данных!\n");                                               
					     syslog(LOG_NOTICE,"%s",status_vector);                                     
					     syslog(LOG_NOTICE,"Приложение завершено с ошибкой.\n");
				...
				//Если подключение к базе денных произошло успешно, начинается транзакция                    
				     ...
				     if (status_vector[0] == 1 && status_vector[1])                               
					     ...
					     printf("Приложение завершено с ошибкой.\n");
				             ...
				             syslog(LOG_NOTICE,"2. Ошибка начала транзакции! \n");
					     syslog(LOG_NOTICE,"%s",status_vector);                                     
				             syslog(LOG_NOTICE,"Приложение завершено с ошибкой.\n");                       
			 ...
			 //Подготовка к выполнению запроса сервером                                        
				 ...
				 //Проверка на ошибку                                                         
				     if (status_vector[0] == 1 && status_vector[1])                               
					     ...
					     syslog(LOG_NOTICE,"3. Ошибка подготовки к выполнению запроса!\n");
					     syslog(LOG_NOTICE,"%s",status_vector);                                     
				             syslog(LOG_NOTICE,"Приложение завершено с ошибкой.\n");                       
			...
			//Затем завершить транзакцию и выйти                                                  
				...
				//Проверка на ошибку                                                         
				     if (status_vector[0] == 1 && status_vector[1])                               
					     ...
					     syslog(LOG_NOTICE,"Ошибка завершения транзакции!\n");                                               
					     syslog(LOG_NOTICE,"%s",status_vector);                                     
				             syslog(LOG_NOTICE,"Приложение завершено с ошибкой.\n");                       
